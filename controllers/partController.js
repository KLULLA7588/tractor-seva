...
export async function createPart(req, res, next) {
  try {
    ...
    if (image_id) {
      ...
      let xVal = null;
      let yVal = null;
      // radius is a PERCENTAGE of the image's rendered width (not raw px) —
      // use parseFloat so decimal values (e.g. 2.3) aren't truncated to 0.
      let finalRadius = radius !== undefined ? parseFloat(radius) : 2;
...
export async function bulkCreateParts(req, res, next) {
  ...
  try {
    ...
    // Default hotspot size for bulk-imported parts — PERCENTAGE of image
    // width (matches HotspotEditor.jsx / DiagramViewer.jsx default of 2%).
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (existing && !existing.linkedToThisImage) {
        ...
        if (image_id) {
          const coordId = generateUUID();
          linkOnlyValues.push([uuidToBuffer(coordId), uuidToBuffer(existing.id), uuidToBuffer(image_id), DEFAULT_RADIUS]);
      ...
      if (image_id) {
        const coordId = generateUUID();
        coordValues.push([uuidToBuffer(coordId), uuidToBuffer(partId), uuidToBuffer(image_id), DEFAULT_RADIUS]);
      }
...
export async function updateHotspot(req, res, next) {
  try {
    ...
    // radius is a PERCENTAGE — use parseFloat so decimals (e.g. 2.3) survive.
    const finalRadius = radius !== undefined ? parseFloat(radius) : existing[0].radius;
