...
export async function getSections(req, res, next) {
  try {
    ...
    // Fetch ALL subsections for this harvester in ONE query, then group
    // them by parent in memory — instead of firing one query per main
    // section in a loop (which is what was causing the slow / timed-out
    // response on harvesters with many sections). Output shape is
    // identical to before: each main section still gets a `subsections`
    // array nested on it.
    const [allSubsections] = await pool.query(
      `SELECT id, harvester_id, parent_id, name, icon, created_at
       FROM sections
       WHERE harvester_id = ? AND parent_id IS NOT NULL
       ORDER BY created_at ASC`,
      [uuidToBuffer(harvester_id)]
    );
    ...
    for (const sub of allSubsections) {
      const parentKey = bufferToUuid(sub.parent_id);
      if (!subsByParent[parentKey]) subsByParent[parentKey] = [];
      subsByParent[parentKey].push(convertRow(sub, SECTION_FIELDS));
    }

    const sectionsWithSubs = mainSections.map((section) => {
      const converted = convertRow(section, SECTION_FIELDS);
      converted.subsections = subsByParent[converted.id] || [];
      return converted;
    });
